...
export async function handleServerStartWorker(context: Context<"server.start", "worker">): Promise<CallbackResult> {
  ...
  const login = payload.comment?.user?.login ?? payload.sender?.login ?? payload.issue?.user?.login ?? null;
  if (!login) {
  ...
  if (!cachedToken) {
    ...
    return { status: 202, reason: "OAuth authorization requested. Please follow the comment link." };
  }
